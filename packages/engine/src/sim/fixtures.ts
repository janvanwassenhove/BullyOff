...
export function standardLineup(): PlayerSetup[] {
  ...
  const home = shape.map(([x, y], i): PlayerSetup => ({ id: i + 1, team: 0, x, y, heading: 0 }));
  const away = shape.map(([x, y], i): PlayerSetup => ({ id: i + 12, team: 1, x: -x, y: -y, heading: dmath.PI }));
  return [...home, ...away];
}

export function sandboxSetup(profile: ProfileId = 'mens', surface: SurfaceState = 'watered'): MatchSetup {
  return { profile, surface, players: standardLineup(), frameEvery: 1 };
}
