...
// a 20-rated athlete so the kinematics match the raw profile numbers
const ELITE = attributesFor('MID', 20);

describe('player kinematics', () => {
  it('accelerates to top speed under limited acceleration and stops faster than it starts', () => {
    const p = createPlayer(1, 0, 0, 0, 0, MENS.player, 'MID', ELITE);
    p.wantDir = { x: 1, y: 0 }; p.wantEffort = 1;
    let ticksToTop = 0;
    let prev = -1;
    // run until speed stops increasing (fatigue trims the ceiling slightly during the run-up)
    while (p.vel.x > prev + 1e-6) { prev = p.vel.x; stepPlayer(p, DT); ticksToTop++; if (ticksToTop > 200) break; }
    ...
    p.wantEffort = 0;
    let ticksToStop = 0;
    while (p.vel.x > 1e-9) { stepPlayer(p, DT); ticksToStop++; }
  ...
  it('sprinting drains stamina; idling recovers it; fatigue caps speed', () => {
    const p = createPlayer(1, 0, 0, 0, 0, MENS.player, 'MID', ELITE);
    p.wantDir = { x: 1, y: 0 }; p.wantEffort = 1;
    for (let i = 0; i < 20 * 60; i++) stepPlayer(p, DT); // one minute flat out
    expect(p.stamina).toBeLessThan(0.8);
    const tiredTop = p.vel.x;
    expect(tiredTop).toBeLessThan(MENS.player.maxSpeed);
    p.wantEffort = 0;
    for (let i = 0; i < 20 * 60; i++) stepPlayer(p, DT);
