...
export function seasonFinances(w: World): void {
  ...
  for (const c of Object.values(w.clubs)) {
    ...
    c.finances.membershipIncome = members * 320;
    c.finances.sponsorIncome = Math.round((c.tier === 1 ? 45000 : 15000) * (0.7 + c.reputation / 100) + rng.gaussian(0, 5000));
    c.finances.facilityCosts = 18000 + c.facilities * 9000;
    c.finances.travelCosts = 6000 + rng.int(3000);
    c.finances.coachingCosts = c.tier === 1 ? 32000 : 14000;
    const net = c.finances.membershipIncome + c.finances.sponsorIncome - c.finances.facilityCosts - c.finances.travelCosts - c.finances.coachingCosts;
    c.finances.balance += net;
    // invest / cut back
    if (c.finances.balance > 120000 && c.facilities < 5 && rng.chance(0.4)) { c.facilities++; c.finances.balance -= 60000; }
    if (c.finances.balance < -40000 && c.facilities > 1) { c.facilities--; c.finances.balance += 20000; }
    c.finances.balance = clamp(c.finances.balance, -200000, 2_000_000);
  }
}
