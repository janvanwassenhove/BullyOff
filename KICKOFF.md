...
## Current phase: **Phase 11b — handedness — SHIPPED 2026-08-31** (engine **0.9.0**, sandbox golden `2c22986d5b9e092a`, `docs/handoff/phase-11b.md`): every stick is right-handed, so the side a ball sits on scales receiving, striking and carrying, the tackle side decides clean-or-foul, and `toReverse` shepherds a carrier onto his backhand (`jockeySpot`). Measured in play: 78 % clean control on the forehand against 66 % on the reverse; from the open stick side 21 % of tackles won against 12 %, with 15 % fouls against 24 %. `gkSaveScale` (men) 2.05 → 1.84 — the provisional knob shrinking as the mechanism it stood for landed. `pnpm check` 251 tests green. **Next per `hockey-systems.md` §9: naturalness** (`adaptive-play.md` layer D — commitment, softmax, anticipation, timed runs), with §5 PC defence systems and §7 entry patterns still open. Note the numbering clash: §9 calls naturalness "phase 12", but that number went to the competitions of Europe — refer to the remaining adaptive-play phases by name (naturalness → reads → fitting → season), not by number.

Previously: v1.0.0 tagged 2026-08-30; Phase 12 competitions of Europe shipped 2026-08-31 (`docs/handoff/phase-12.md`); Phase 10.6 the review round (`docs/handoff/phase-10.6.md`).
...
Phase 10.1 (`docs/handoff/phase-10.1.md`): Jan's second look — intro film as six generated stills with captions, all engineering jargon rewritten into hockey language (NL/EN/FR), tempo measured and tuned (passes arrive at 6.5–10 m/s, broadcast camera by default, ball speed trail; engine **0.7.0**, golden `cec18ab670a0562b`, `tempo.test.ts` guards it), rulebook scenes: spatial rules on the pitch renderer, stick/height/card rules as side-elevation figures (lib/ruleFigures.ts); the hub tells you to start a career instead of showing dead menus. **Phase 10.2**: the coach picks the team sheet on the squad screen (save **v4**), signs it before kick-off, and names the PC battery and captain on the tactics screen so they hold for simulated match days too. **Phase 10.3**: the tactics board draws the real pitch with the press and back lines where the AI puts them (`pressLineM` / `backLineM` are now one source), the corner routine is picked in-match from the men actually on the pitch, and a scouting report on the opponent (`insight.scoutOpponent`) argues for a counter. `pnpm check` 217 tests green. The owed 96-match calibration re-baseline was paid in **Phase 10.5**. **Phase 10.4** closed the phone pass: every screen audited at 375/768 px, the coach view rebuilt for phone (pitch-first column, subs reachable, thumb decisions), grid blowouts and squeezed name columns fixed, coarse-pointer tap targets and safe-area insets added.
...
2. ~~**The phone pass**~~ — done in Phase 10.4 (2026-08-30).
...
32. ~~**Phase 11 merged**~~ — **merged 2026-08-22 on Jan's call** (`docs/handoff/phase-11.md`): pressing systems as data + the assignment model. It carried a known regression: mens goals per match outside the calibration band because the defensive half landed without §7's attacking patterns. **Resolved in Phase 10.5 (2026-08-30)**: the attacking half landed as mechanisms (entry passes that beat the recycle, forwards in the D, win-the-corner that works, D clearances) and the gate is met — mens 5.20 in [4.86, 5.94], PC share 0.34, corners 10/match; women in band too. See `docs/rules/calibration.md` §0.8.0. Do not reach for the fitting layer first: that would fit weights to a half-built model. Remaining order after §7: handedness (11b) → naturalness (12) → reads (13) → fitting (14) → season (15).
