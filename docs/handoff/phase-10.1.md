...
`lib/ruleClips.ts` authors one scene per rule as keyframes in metres and seconds (attackers +x, first player of each team = keeper) and compiles it into a replay log; `RulebookView.vue` plays the selected rule on the real pitch renderer in a stage at the top (looping via `play()` at `lastTick`), cards below select a rule, deep links from the report still land on the card. Fifteen scenes: feet · dangerous · back-stick · obstruction · stick tackle · five metres · self-pass in the 23 · PC breach · PC first hit · stroke · circle rule · penalty corner · cards · rolling subs · aerial. Events in the scene drive the renderer's own banners/whistles (PENALTY CORNER, GOAL). No hand-drawn diagrams, consistent with the key-moment thumbnails.
