...
const props = withDefaults(defineProps<{
  ...
  fit?: 'contain' | 'cover';
}>(), { colours: () => [0x1d3557, 0xe63946], camera: 'broadcast', overlay: 'none', mode: 'tactical', coachTeam: 0, live: false, autoPlay: true, autoPauseOn: () => [], fit: 'contain' });
...
async function mount(): Promise<void> {
  ...
  view.value?.destroy();
  const v = await createMatchView(canvas.value, props.log, { mode: props.mode, camera: props.camera, overlay: props.overlay, homeColour: props.colours[0], awayColour: props.colours[1], live: props.live, coachTeam: props.coachTeam, autoPauseOn: props.autoPauseOn, fit: props.fit });
